...
public class Pistola implements Arma{
    ...
    public Pistola(AssetManager assetManager,Vector3f posicionJugador, Node armas){
        arma = assetManager.loadModel("Models/pistola.j3o");
    ...
    public void disparar(float tpf,Camera cam,Node enemigosNode,float danioExtra) {
          ...
          if (balas > 0 && tiempoDesdeUltimoDisparo() >= .6f) {
            // Realizar el disparo
            balas--;
    ...
    public void recargar(float tpf) {
        if (balas == 0 && !recargando() && tiempoDesdeUltimaRecarga() >= 4f) {
            // Iniciar la recarga
