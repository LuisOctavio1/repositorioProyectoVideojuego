 ...
 * @author PC NASUA
 */
public class EnemigoSeguidor implements Enemigo{
        int vida;
    ...
    public EnemigoSeguidor(AssetManager assetManager,Vector3f posicionInicial,Vector3f posicionFinal){
        vida = 150;
        velocidad = (float) 25;
    ...
    if (enemigo != null && enemigo.getLocalTranslation().distance(posicionFinal) > 2.0f) {
            avanzar(posicionFinal, tpf);   
            return 0;
        }else{
            if (tiempoDesdeUltimoAtaque >= 2.5f) {
    ...
    public void actualizarPosicionJugador(Vector3f posicionJugador){
        this.posicionFinal = posicionJugador;
    }
}
