 ...
 * @author PC NASUA
 */
public class Sniper implements Arma{
    ...
    public Sniper(AssetManager assetManager,Vector3f posicionJugador, Node armas){
        arma = assetManager.loadModel("Models/sniper.j3o");
    ...
    public void disparar(float tpf,Camera cam,Node enemigosNode,float danioExtra) {
          ...
          if (balas > 0 && tiempoDesdeUltimoDisparo() >= .9f) {
            // Realizar el disparo
            balas--;
    ...
    public void recargar(float tpf) {
        if (balas == 0 && !recargando() && tiempoDesdeUltimaRecarga() >= 10f) {
            // Iniciar la recarga
